...
export class Users implements OnInit {
  ...
  generateUsername() {
    const first = this.formData().first_name.trim().toLowerCase()
      .normalize("NFD").replace(/[\u0300-\u036f]/g, "") // Supprime les accents
      .replace(/\s+/g, '-'); // ✅ Remplace TOUS les espaces par des tirets
    
    const last = this.formData().last_name.trim().toLowerCase()
      .normalize("NFD").replace(/[\u0300-\u036f]/g, "")
      .replace(/\s+/g, '-');

    if (first && last) {
      this.formData.update(d => ({ ...d, username: `${first}.${last}` }));
    }
  }
...
if (this.searchTerm().trim()) params['search'] = this.searchTerm().trim();    this.userService.loadUsers(this.currentPage(), params).subscribe({
  ...
  if (!d.first_name.trim()) {
    ...
    e.first_name = 'Minimum 2 caractères';
  }

  // Nom
  if (!d.last_name.trim()) {
    e.last_name = 'Le nom est obligatoire';
  } else if (d.last_name.trim().length < 2) {
    e.last_name = 'Minimum 2 caractères';
  }

  // ✅ Email optionnel : validé SEULEMENT s'il est renseigné
 ...
 openCreateModal() {
  ...
  this.errors.set({});
  this.formData.set({
    first_name: '', last_name: '', username: '', email: '', phone: '', // ✅ AJOUTÉ
 ...
 openEditModal(user: User) {
  ...
  onSubmit() {
    ...
    const data: any = { 
      ...
      // ✅ ENVOIE 'null' SI VIDE, PAS UNE CHAÎNE VIDE ""
      username: d.username.trim() === '' ? null : d.username.trim()
    };
