...
export class Registrations implements OnInit {
  ...
  errors = signal({
    first_name: '', last_name: '', email: '', formation_id: '', academic_year_id: '', initial_payment: '', custom_tuition: ''
  });
  ...
  displayedTuition = computed(() => {
    ...
    if (f.has_promo && f.custom_tuition !== null && f.custom_tuition > 0) {
      return f.custom_tuition;
    }
    const formation = this.selectedFormation();
    return formation?.tuition_fees || 0;
  });
  ...
  openCreateModal() {
    ...
    this.errors.set({ first_name: '', last_name: '', email: '', formation_id: '', academic_year_id: '', initial_payment: '', custom_tuition: '' });
  ...
  validateCustomTuition(): boolean {
    const d = this.formData();
    if (d.has_promo && (d.custom_tuition === null || d.custom_tuition <= 0)) {
      this.errors.update(e => ({ ...e, custom_tuition: 'Le montant promo est obligatoire' })); return false;
    }
    this.errors.update(e => ({ ...e, custom_tuition: '' })); return true;
  }
  ...
  getStatusLabel(status: string): string {
    const labels: Record<string, string> = {
      unpaid: 'Non payé', partial: 'Partiel', paid: 'Soldé', confirmed: 'Validé'
    };
